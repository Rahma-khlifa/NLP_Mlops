...
def main():
    ...
    # --- LOGIQUE DE GESTION DE LA DÉRIVE (Simplifiée pour CI/CD conditionnel) ---
    # Si ce script est exécuté, cela signifie qu'une dérive a été détectée et qu'un ré-entraînement est nécessaire.
    # Le ré-entraînement se fait sur la fusion des deux versions de données (V1 + V2).
    ...
    try:
        ...
        # Définition des chemins de données à utiliser
        DATA_V1_PATH = preprocess.DATA_DIR / 'version1.xlsx'
        DATA_V2_PATH = preprocess.DATA_DIR / 'version2.xlsx'
        
        print("\n--- LOGIQUE DE RÉ-ENTRAÎNEMENT ---")
        print("❌ Dérive détectée (Script exécuté). Fusion des données V1 et V2 pour un ré-entraînement sur un jeu de données plus riche.")
        
        # Charger les deux DataFrames
        df_v1 = preprocess.load_and_explore_data(DATA_V1_PATH)
        df_v2 = preprocess.load_and_explore_data(DATA_V2_PATH)
        
        # Fusionner les deux DataFrames (suppose que les colonnes sont identiques)
        df = pd.concat([df_v1, df_v2], ignore_index=True).drop_duplicates()
        ...
        # Utiliser le DataFrame 'df' chargé/fusionné ci-dessus
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Determine data path (Lignes commentées car la logique de chargement/fusion est déplacée)
        ...
        print("🧪 PREPROCESSING - Integrated in Training Pipeline")
        print("="*80)
        # df = preprocess.load_and_explore_data(data_path) # Ligne supprimée, df est déjà chargé/fusionné
